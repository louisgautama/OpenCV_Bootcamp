...
clas_name = []
whT = 320 #width, height, target dimension
confThreshold = 0,5 
nmsThreshold = 0.3
model_configuration = "yolov3/yolov3-320.cfg"
model_weights = "yolov3/yolov3-320.weights"
...
def findObjects(outputs, img):
    ...
    bbox = [] #boundary box
    classIDs = []
    confs = [] #condidence score

    for output in outputs:
        for det in output:
            scores = det[5:]
            classId = np.argmax(scores) #give max
    ...
    for i in indices:
        ...
        cv2.putText(img, f"{clas_name[classIDs[i]].upper()} {int(confs[i] *100)}%" (x,y-10), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.6, (255,0,255), 2)
